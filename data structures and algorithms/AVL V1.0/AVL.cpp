...
NodeAVL* leftRotation(NodeAVL* node)	//p is critical node
{
	...
	printf("nodul pt care se face rotatia este ");
		printf("%d(%d) \n", node->key, node->echi);

	heavyChild = node->right;

	printf("HeavyChild este:");
	printf("%d(%d) \n", heavyChild->key, heavyChild->echi);
	...
	printf("Timpul rotatiei este: ");
	stop = clock();
	printf("%lf", double((stop - start)) / CLOCKS_PER_SEC, " SECONDS ");
	printf("\n\n");
	node = heavyChild;
	return node;
}
...
{
	printf("Atentie rotatie dubla stanga!!\n");
...
{
	printf("Atentie rotatie dubla dreapta!!\n");
